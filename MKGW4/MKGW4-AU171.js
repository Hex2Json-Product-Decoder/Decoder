...
var typeCodeArray = ["ibeacon", "eddystone-uid", "eddystone-url", "eddystone-tlm", "bxp-devinfo", "bxp-acc", "bxp-th", "bxp-button", "bxp-tag", "pir", "other", "tof","bxp-s"];
...
var beaconUploadNumberType = ['8','16','24','32'];
...
function handlePayload(value, msgType, index) {
    ...
    if (len > 11) {
        ...
        } else if (data.flag == '2044') {
            // Beacon upload numbers
            return parseBeaconReport(deviceDataIndex, deviceDataLength, deviceDataArray, data);
        ...
        } else if (data.flag == '30b3') {
            // Offline data uploaded
            return parseOfflineUploadData(deviceDataIndex, deviceDataLength, deviceDataArray, data);
        }
    }
    return value;
}
...
function parseBeaconReport(deviceDataIndex, deviceDataLength, deviceDataArray, data) {
    var beaconReport = {};
    for (; deviceDataIndex < deviceDataLength;) {
        var paramTag = parseInt(deviceDataArray[deviceDataIndex], 16);
        deviceDataIndex++;
        ...
        deviceDataIndex += 2;
        if (paramTag == 0) {
            beaconReport.number = beaconUploadNumberType[parseInt(deviceDataArray[deviceDataIndex], 16)];
        } 
        deviceDataIndex += paramLength;
    }
    data.beaconReport = beaconReport;
    return JSON.stringify(data);
}
...
function parseFilterIbeacon(deviceDataIndex, deviceDataLength, deviceDataArray, data) {
    ...
    for (; deviceDataIndex < deviceDataLength;) {
        ...
        deviceDataIndex += 2;
        if (paramTag == 0) {
            filterIbeacon.regular_expression = hexStrToString(deviceDataArray.slice(deviceDataIndex, deviceDataIndex + paramLength));
        } 
        deviceDataIndex += paramLength;
    }
...
function parseOfflineUploadData(deviceDataIndex, deviceDataLength, deviceDataArray, data) {
    var fixData = {};
    for (; deviceDataIndex < deviceDataLength;) {
        var paramTag = parseInt(deviceDataArray[deviceDataIndex], 16);
        deviceDataIndex++;
        ...
        if (paramTag == 0) {
            ...
            //已上传数据条数
            fixData.uploadedNumbers = parseInt(deviceDataArray[deviceDataIndex], 16);
        } else if (paramTag == 2) {
            //剩余数据条数
            fixData.remainingNumbers = parseInt(deviceDataArray[deviceDataIndex], 16);
        }
        deviceDataIndex += paramLength;
    }
    data.offlineUploadedData = fixData;
    return JSON.stringify(data);
}
...
function parseBXPS(deviceItem, paramTag, deviceDataArray, deviceDataIndex, paramLength) {
    if (paramTag == 0x0A) {
        ...
        deviceItem.hallTriggerEventCount = parseHexStrArraytoInt(deviceDataArray.slice(deviceDataIndex, deviceDataIndex + paramLength)) + "mV";
    } else if (paramTag == 0x0C) {
        deviceItem.motionTriggerEventCount = parseHexStrArraytoInt(deviceDataArray.slice(deviceDataIndex, deviceDataIndex + paramLength));
    } else if (paramTag == 0x0D) {
        var axis_data_array = deviceDataArray.slice(deviceDataIndex, deviceDataIndex + paramLength);
